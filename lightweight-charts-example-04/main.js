...
for (let key in lines) {
  ...
  lineSeries.setData(lines[key].values.slice(-renderedPointsCount));
...
function setChartData() {
  ...
  candlestickSeries.setData(klines.slice(-renderedPointsCount));
  let startTime = candlestickSeries.data()[0].time;
  let newMarkers = markers.filter((item) => {
  ...
  for (let key in lineSeriesGroup) {
    lineSeriesGroup[key].setData(
      lines[key].values.slice(-renderedPointsCount)
    );
  }
}
